...
class DrivingScreen extends React.Component {
    ...
    _getdata() {
        let record = [];
        db.transaction((tx) => {
            tx.executeSql('SELECT * FROM Xuphat where loai_xe = 1 ORDER BY ten_loi asc limit 0,20', [], (tx, results) => {
    ...
    _onEndReached() {
        let new_record = [];
        db.transaction((tx) => {
            tx.executeSql('SELECT * FROM Xuphat where loai_xe = 1 ORDER BY ten_loi asc limit ' + this.state.page + ',20', [], (tx, results) => {
                let len = results.rows.length;
                if (len > 0) {
                    for (let i = 0; i < len; i++) {
                        let row = results.rows.item(i);
